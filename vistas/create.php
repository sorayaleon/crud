...
// Proceso el formulario al pulsar el botón aceptar de esta ficha
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["aceptar"]){
    ...
    if(empty($nombreVal)){
        $nombreErr = "Por favor introduzca un nombre válido. Sólo caracteres alfabéticos.";
    ...
    // Chequeo los errores antes de insertar en la base de datos
    if(empty($dniErr) && empty($nombreErr) && empty($passwordErr) && empty($emailErr) && 
        ...
        // creo el controlador de alumnado
        $controlador = ControladorAlumno::getControlador();
